...
- **This does not make `testNewReportsAccessibilityAudit` green, and it is not meant to.** The rule covers 3 of that test's 6 contrast findings. The remaining three — `Monthly essentials`, `To reach 3 months` and `Contributing Accounts` — are genuinely visible, crisp, fully on-screen text. They belong to the `form-section-header` sampler false-positive class already documented elsewhere in the file. **They are deliberately NOT exempted here.** Whether to excuse that class is a separate owner decision and was not authorised with this one. A green run on that test is therefore not the success criterion for this change; the success criterion is that the three chrome-occluded findings stop being reported while everything genuinely on screen keeps being audited.
