...
## DEC-024: The paywall's decorative glyphs are brand green, and StoreKit's black CTA label is accepted

- Status: Accepted (2026-09-14, Rahul — owner). **Extends DEC-023** to the paywall's non-CTA marketing glyphs. Reported by the owner from a device screenshot.
- Decision, part one: the `sparkles` hero glyph (`PaywallView.swift:245`) and the six `checkmark.circle.fill` feature discs (`PaywallView.swift:267`) move from `VColors.primaryOnSurface` (`#17604A`) to `VColors.primary` (`#3FCFA4`). The paywall now carries one green on its content, not two.
- Why this reverses DEC-023's reasoning on these specific glyphs: DEC-023 argued the discs should stay `#17604A` because the checkmark is knocked out of the disc in the page colour, so brand green drops that check to 1.97:1. The ratio claim is correct and is not disputed. The owner's decision is that visual consistency wins here. It costs nothing in accessibility terms because both glyphs are `.accessibilityHidden(true)` and each disc sits immediately beside a text label that states the same feature in words — no information is carried by the glyph alone, so a low-contrast check conveys nothing a user could miss.
- No DEC-012 exemption was added and none is needed: the audit never samples these elements. They are hidden from the accessibility tree, which is the tree the audit walks. The policy links keep `#17604A` via `.subscriptionStorePolicyForegroundStyle` — they are foreground *text* on a near-white page, DEC-012 does not cover foreground text, and pale green there is a real legibility regression.
- Decision, part two — StoreKit's black purchase-button label stays black. The owner was offered two options and chose to leave it: (a) accept the black label, or (b) drop `SubscriptionStoreView`'s built-in CTA and drive the purchase from our own `Button`, as the lifetime button already does, which would render white. Option (a) was chosen.
- Why there is no third option: the iOS 26 SDK exposes nine `subscriptionStore*` view modifiers — `ButtonLabel`, `ControlBackground`, `ControlIcon`, `ControlStyle`, `OptionGroupStyle`, `PickerItemBackground`, `PolicyDestination`, `PolicyForegroundStyle`, `SignInAction`. None sets the purchase button's label colour, and `.foregroundStyle` is not honoured on that control. StoreKit derives the label from the tint's luminance and picks black on `#3FCFA4`. Measured `#000000` on `#3ECEA3` at 10.56:1.
- The accepted consequence, stated plainly because it is visible on the screen: the paywall shows two CTAs on the identical brand-green fill with different label colours — StoreKit's "Try It Free" in black, our "Or buy Vittora Pro Lifetime" in white directly beneath it. This is a cosmetic inconsistency the owner has seen and accepted.
- Build impact: `Vittora/Features/Paywall/PaywallView.swift` only — two `foregroundStyle` values and their comment blocks. No test file changed, no exemption added or removed, no new dependency.
- Revisit if: option (b) becomes worth its cost, in which case the lifetime button's purchase path is the template; or Apple ships an API for the purchase-button label colour; or DEC-012 is reopened.

## DEC-025: `paywall-lifetime-button` takes a DEC-012 exemption, as DEC-023 prescribed

- Status: Accepted (2026-09-14). Executes the remedy DEC-023 wrote down in advance. Does not reopen DEC-012.
- What happened: after `636169ea` put the paywall on brand green, `testPaywallAccessibilityAudit` began failing intermittently on `Contrast failed`. One run in the 1.7.0 UI-core leg failed; an immediately following single-test run passed on identical code.
- The element, identified rather than guessed: temporary logging in the audit handler printed every issue with its label, identifier and frame. The finding is `paywall-lifetime-button`, "Or buy Vittora Pro Lifetime for $99.99, once", frame `(32, 659, 329, 50)` — on screen and unoccluded. White on `#3FCFA4` is 1.97:1.
- Why it is intermittent: the button's position varies between runs. When it lands behind the navigation bar the DEC-020 top-side viewport rule excuses it; when it lands clear, the finding stands. The pairing was always wrong after `636169ea` — only the audit's ability to see it varied.
- What it is NOT: not the DEC-024 glyph change. The `sparkles` hero and the six `checkmark.circle.fill` discs are `.accessibilityHidden(true)`, and the audit walks the accessibility tree, so they never appear in its findings at all. The logging confirms this — neither glyph is in the issue list. DEC-023's claim on that point was correct.
- Decision: add `paywall-lifetime-button` to the DEC-012 exemptions, anchored to the **identifier**. DEC-023 named this identifier and this remedy before the failure occurred: "If the audit ever reaches it unoccluded it will fail on this pairing, and the fix is to add paywall-lifetime-button to the exemptions in AccessibilityAuditUITests deliberately, not to change the colour."
- Why the identifier and not the label: the label is the user-visible price string. A price change must not silently widen the exemption to a different control, nor void it and resurrect the flake.
- On house rule 9 ("never change the audit config to make a failing check pass"): this is an exemption, and it is recorded as one rather than glossed. It is accepted because DEC-012 is the owner's standing decision that white-on-brand-green CTAs ship, because DEC-023 prescribed this exact entry in advance rather than it being invented to silence a red leg, and because the alternative that would pass AA — a black label at 11.4:1, matching what StoreKit derives for its own button directly above — was offered to the owner and not taken. The owner asked for white.
...
## DEC-027: The paywall's policy links take a DEC-012 exemption — the sampler measures them against the subscribe button

- Status: Accepted (2026-09-15). Completes what DEC-025 started; DEC-025 alone did not make the CI leg green.
- What DEC-025 got wrong: it identified `paywall-lifetime-button` as the failing element and exempted it. That element is real and the exemption is correct, but it was not what kept CI red. The evidence for DEC-025 was three green runs on **iPhone 16**; CI runs **iPhone 17 Pro Max**. The audit is device-sensitive, which the project already knew, and a green run on the wrong device proved nothing.
- The actual finding, named rather than inferred: on iPhone 17 Pro Max the audit keeps exactly three contrast issues — `Terms of Service`, `and`, and `Privacy Policy` — at frames `y=836.7, h=17.3`, with measured ratios of **1.58, 1.60 and 1.58**. They were invisible in the failure output because the contrast branch exits at `return isChartMark`, before `logAuditIssue` runs, so nothing was ever logged for them.
- Why the ratio is 1.58: cropping CI's own App Screenshot attachment at exactly that rect shows the "Try It Free" capsule and no link text at all — 40,469 pixels of `#3ECDA2`. The links are scrolled elsewhere; only their reported frames land on the CTA. The sampler therefore compares `#17604A` against brand green. Where the links are genuinely painted they are `#17604A` on the near-white sheet at 7.48:1, which is exactly why `.subscriptionStorePolicyForegroundStyle` pins that colour. This is a sampler artifact, not a legibility miss.
- Decision: exempt those three identifiers, screen-scoped to `navigationBars["Vittora Pro"]`.
- What was rejected on the way, and why it is worth recording: two frame-intersection arms were added first — one for `paywall-lifetime-button`, one for a guessed `Subscription Store View Standard Picker Style Subscribe Button` identifier — on the theory that the failing node was an unlabelled inner node. Three runs on iPhone 17 Pro Max with those arms in place still failed 3/3. They were discarded rather than left in, because an exemption that cannot be shown to be load-bearing is the rot DEC-018 warned about.
- Method note, because it changed the cost of this class of bug: the failure was diagnosed by downloading CI's `xcresult` artifact, exporting its attachments, and reading the element screenshot and issue description. The device was then reproduced locally (`iPhone 17 Pro Max`), turning a 55-minute CI round trip into a 30-second one. Reproduce the device before theorising.
- Evidence: 3/3 green runs of `testPaywallAccessibilityAudit` on iPhone 17 Pro Max, plus the full `AccessibilityAuditUITests` suite on that device, plus unit and UI core on iPhone 16.
- Build impact: `VittoraUITests/AccessibilityAuditUITests.swift` only. No production code, no assertion weakened, no `XCTSkip`.
- **Second half, added after the first attempt still failed CI.** Exempting the policy links alone left two findings. CI's artifact named them: the element screenshot is the whole "Try It Free" capsule at 1080x150px — exactly `Subscription Store View Standard Button`'s 360x50pt frame at 3x — plus a 534x47px sliver of its bottom edge meeting the page. Both arrive as bare `SwiftUI.AccessibilityNode`s with no label and no identifier. The label inside that capsule is BLACK on brand green at 10.56:1 and passes; what the sampler measures is the green fill against the page, which is the DEC-012 pairing. Apple draws the control and exposes no API to restyle it, exactly as DEC-019 records for its caption. Anchored to `Subscription Store View Standard Button` and `Subscription Store View Button` by identifier and by frame, screen-scoped to the paywall.
- The identifiers were read from the live accessibility tree with a throwaway dump test, not guessed. An earlier attempt invented `Subscription Store View Standard Picker Style Subscribe Button` — that identifier does not exist, the arm matched nothing, and it cost a CI cycle.
- **What local testing could and could not prove here.** The configuration matrix ended: iPhone 16 passes (DEC-025's false evidence); iPhone 17 Pro Max / iOS 26.4 reproduced the policy-link findings; iPhone 17 Pro Max / iOS 26.2 — CI's exact device AND runtime — passes. Only CI reproduces, because CI builds with **Xcode 26.3** and this machine has 26.6. So this half of DEC-027 is verified by CI, not locally; the local runs prove only that nothing regressed. Diagnosis came from CI's own xcresult artifact, which is the reliable tool for this class.
- Revisit if: Apple gives the policy links stable frames, or `subscriptionStorePolicyForegroundStyle` stops pinning `#17604A`, or Apple's identifiers for the subscribe control change.

## DEC-028: iOS 27's swipe-actions API is not adopted — the defect it would fix is already fixed

- Status: Accepted (2026-09-18). Phase D of the 1.7.1 iOS 27 work, dropped after checking the premise rather than after building it.
- What was planned: iOS 27 adds `swipeActions(edge:allowsFullSwipe:content:onPresentationChanged:)` and `swipeActionsContainer()`. The `onPresentationChanged` callback reports when a swipe container opens and closes, which is what was missing when a swipe-to-delete confirmation anchored to the wrong place on iPad — the defect reported with screenshots in the 1.7.0 cycle.
- Why it is not adopted: that defect no longer exists, and it was not fixed by anchoring better. It was fixed by moving from `confirmationDialog`, which anchors to a source rect on iPad, to `.alert`, which is a centred modal with nothing to anchor. `grep -c confirmationDialog` across `Vittora/` is now **0**. All ten swipe sites set a `…ToDelete` item and present a list-level alert.
- So the callback has no defect behind it. What it would buy is polish: dismissing an open swipe row when the alert is dismissed.
