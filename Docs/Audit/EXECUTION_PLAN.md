...
| C2 | FUNCTIONAL-21 | `Features/Onboarding/...` | Soft permission-priming step (after value, not first launch); defer the system prompt to feature-enable. | `OnboardingViewModelTests` covers new step. | S | **Merged** |
...
| L9 | TESTING-10 | `VittoraUITests/*` + `Makefile` | **Stabilize flaky UI tests** (transaction filter, transfer flow, etc.) — proper waits/hittability fixes; remove reliance on CI `-retry-tests-on-failure` stopgap (#28). | UI suite green 3× locally + CI without retry. | M | **Merged #31** |
| L9b | TESTING-10 | `VittoraUITests.swift`, `TransactionFlowUITests.swift` | Harden residual flakes: `testLargeTextDoesNotBreakLayout`, `testCanSearchAndFilterTransactions` (row polling, filter-sheet dismiss, foreground wait). | Retry-free CI green on tip. | S | **Merged #34** |
| L9c | TESTING-10 | `OnboardingFlowUITests` + `Makefile` | Re-enable onboarding UI in CI gate after L9b holds (UITestSupport waits, remove CI skip). | Onboarding suite green in gate. | S | **In PR** |
