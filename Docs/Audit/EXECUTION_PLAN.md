...
- **Status:** **HOLD for review** on `fix/A11-debt-settlement-links`. Schema V5 adds `linkedTransactionIDs`; legacy single link merged on read; `performSettle` appends atomically.
...
|---|---|---|---|---|---|
| B1 | SECURITY-1 | `VittoraApp.swift`, `Core/Security/AppLockService.swift` | Store `lastBackgrounded`; on `.active` lock if `now-lastBackgrounded ≥ lockTimeout`; expose timeout setting (Immediately/1m/5m); make the dead inactivity timer live **or** delete it. | App re-locks after timeout; not before. Extract `shouldLock(backgroundedAt:now:timeout:)` pure fn + unit test boundaries. | M |
