# App Store Metadata — en-US REFRESH (current feature set)

**Why this file exists.** `metadata-en-US.md` was written for v1.0.0 and never
updated. The live listing therefore sells far less than the app does — it has no
mention of the Apple Watch app, widgets, Siri, Spotlight, Handoff, 50/30/20,
emergency fund, subscription audit, cash-flow forecast, PDF export, Year in
Review, Hindi or Spanish. Four releases of work is invisible to every visitor.

This is the replacement text, written against what actually ships in 1.5.0.
Verified feature-by-feature against `develop`, not copied from release plans.

**Unchanged and still binding:** free, no IAP (DEC-008 — Pro is DEC-011 and must
not appear here). Only the App Name is kept as-is; everything else is rewritten.
...
- **The Mac listing needs the same refresh, minus Watch and widgets** — both
  targets exclude `macosx` from `SUPPORTED_PLATFORMS`, so those bullets must be
  cut there. Handoff, Spotlight, Siri/Shortcuts, keyboard navigation and every
  report do ship on Mac and should stay. Year in Review works on Mac too (its
  only `#if os(iOS)` is a cosmetic navigation-title modifier).
- **`metadata-en-IN.md` is stale in the same way** and should get the same
  treatment. Keep its India tax framing, and it can now also mention the India
  compliance tips shipped in 1.4.0.
- **Screenshots are the weaker half of this listing.** The captures in
  `screenshots/marketing/` predate the Watch app, widgets and Year in Review, so
  the images still sell v1.0 even once this text ships. Watch captures exist at
  `screenshots/watch/`.
