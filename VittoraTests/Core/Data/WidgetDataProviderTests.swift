...
struct WidgetDataProviderTests {
    ...
    /// Each provider reads currency from its own suite, so nothing touches `.standard`.
    private func makeProvider() throws -> (WidgetDataProvider, ModelContainer) {
        ...
        let suiteName = "WidgetDataProviderTests.provider"
        let defaults = UserDefaults(suiteName: suiteName) ?? .standard
        defaults.removePersistentDomain(forName: suiteName)
        defaults.set("USD", forKey: AppUserDefaults.StandardKey.currencyCode)
        return (WidgetDataProvider(container: container, defaultsSuiteName: suiteName), container)
    }
