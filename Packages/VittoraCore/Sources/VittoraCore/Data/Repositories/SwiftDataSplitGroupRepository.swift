...
@ModelActor
public actor SwiftDataSplitGroupRepository: SplitGroupRepository {

    // MARK: - Groups

    public func fetchAllGroups() async throws -> [SplitGroup] {
    ...
    public func fetchGroupByID(_ id: UUID) async throws -> SplitGroup? {
        let descriptor = FetchDescriptor<SDSplitGroup>(
    ...
    public func createGroup(_ group: SplitGroup) async throws {
    ...
    public func updateGroup(_ group: SplitGroup) async throws {
        let id = group.id
        let descriptor = FetchDescriptor<SDSplitGroup>(
            predicate: #Predicate { $0.id == id }
        )
    ...
    public func deleteGroup(_ id: UUID) async throws {
        let descriptor = FetchDescriptor<SDSplitGroup>(
            predicate: #Predicate { $0.id == id }
        )
        guard let model = try modelContext.fetch(descriptor).first else {
    ...
    public func fetchExpenses(forGroup groupID: UUID) async throws -> [GroupExpense] {
    ...
    public func fetchExpenseByID(_ id: UUID) async throws -> GroupExpense? {
        let descriptor = FetchDescriptor<SDGroupExpense>(
            predicate: #Predicate { $0.id == id }
        )
    ...
    public func createExpense(_ expense: GroupExpense) async throws {
    ...
    public func updateExpense(_ expense: GroupExpense) async throws {
        let id = expense.id
        let descriptor = FetchDescriptor<SDGroupExpense>(
            predicate: #Predicate { $0.id == id }
        )
    ...
    public func deleteExpense(_ id: UUID) async throws {
        let descriptor = FetchDescriptor<SDGroupExpense>(
            predicate: #Predicate { $0.id == id }
        )
        guard let model = try modelContext.fetch(descriptor).first else {
